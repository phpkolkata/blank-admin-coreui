...
@Component({
  selector: 'app-commission-group-master',
  templateUrl: './commission-group-master.component.html',
  styleUrls: ['./commission-group-master.component.css'],
})
export class DoctorReferralPackageComponent implements OnInit {
  title = 'Doctor Referral Package';
  localUrl = 'doctors/commission-group-master/';
  ...
  onSaveGroup(frm: NgForm) {
    ...
    if (this.selectedGroupEditMode) {
      this.srvShared
        .updateRecord(
          data,
          `doctors/master-group/update/${this.selectedGroupPivotId}`
        )
        .subscribe(
          (res) => {
            if (res && res['success']) {
      ...
      this.srvShared.addRecord(data, 'doctors/master-group/add').subscribe(
        (res) => {
          if (res && res['success']) {
            this.srvAlert.updateSuccess();
          } else {
  ...
  fetchGroup() {
    this.srvShared
      .fetchRecords('doctors/commission-group/fetch', {})
      .pipe(
        map((res) => {
          if (res) {
            const rows = res['rows'];
  ...
  onGroupClick(frm: NgForm) {
    ...
    this.srvShared.fetchRecords('doctors/master-group/fetch', config).subscribe(
      (res) => {
        if (res && res['success']) {
          if (res['rows'][0]) {
